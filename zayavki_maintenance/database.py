...
class Database:
    def __init__(self, db_file):
        ...
        self.create_table()
        self.insert_user("meow", "123")
        self.insert_user("slloika", "456")
        self.insert_user("VIP", "777")
    ...
    def search_tickets(self, query):
        self.cursor.execute("SELECT * FROM tickets WHERE ticket_number LIKE ? OR client LIKE ? OR equipment LIKE ?",
                            ('%' + query + '%', '%' + query + '%', '%' + query + '%'))
        return self.cursor.fetchall()

    def search_by_user(self, client):
        self.cursor.execute("SELECT * FROM tickets WHERE client=?", (client,))
        return self.cursor.fetchall()
